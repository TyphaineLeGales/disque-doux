...
export default function Clean(props: CleanProps) {
  ...
  const getStainPosition = useCallback(
    (faceId: number) => {
      'worklet';
  ...
  const isPointOnRag = useCallback(
    (x: number, y: number) => {
      'worklet';
      const margin = 20;
      const ragBounds = {
      ...
      return (
        x >= ragBounds.left && x <= ragBounds.right && y >= ragBounds.top && y <= ragBounds.bottom
      );
    },
  ...
  const isFaceCleaned = useCallback(
    (faceId: number) => {
      const currentGrid = [grid0, grid1, grid2, grid3][faceId];
      let cleaned = 0;
      for (let i = 0; i < GRID_SIZE; i++) {
          ...
          <View style={StyleSheet.absoluteFill}>
            ...
            <Canvas style={canvasStyle}>
              ...
              {currentFaceIdState === 0 && (
                <Group
                  transform={[
                    { translateX: getStainPosition(0).x },
                    { translateY: getStainPosition(0).y },
                  ]}>
              ...
              {currentFaceIdState === 1 && (
                <Group
                  transform={[
                    { translateX: getStainPosition(1).x },
                    { translateY: getStainPosition(1).y },
                  ]}>
              ...
              {currentFaceIdState === 2 && (
                <Group
                  transform={[
                    { translateX: getStainPosition(2).x },
                    { translateY: getStainPosition(2).y },
                  ]}>
              ...
              {currentFaceIdState === 3 && (
                <Group
                  transform={[
                    { translateX: getStainPosition(3).x },
                    { translateY: getStainPosition(3).y },
                  ]}>
            ...
            {showGreat && <Text style={styles.greatText}>Great!</Text>}
            <Animated.Image
